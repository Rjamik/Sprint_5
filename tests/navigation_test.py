...
class TestNavigation:
    ...
    def test_navigation_from_account_to_constructor(self, driver):
        ...
        driver.find_element(*Locators.constructorButton).click()
    ...
    def test_navigation_from_account_to_constructor_use_click_logo(self, driver):
        ...
        WebDriverWait(driver, timeout=5).until(EC.visibility_of_element_located((By.XPATH, Locators.enterLink)))

        assert "login" in driver.current_url

        driver.find_element(*Locators.StellaBurgerLogo).click()
        WebDriverWait(driver, timeout=5).until(EC.visibility_of_element_located(Locators.makeBurgerText))

        assert driver.current_url == 'https://stellarburgers.nomoreparties.site/'

    def test_navigation_tab_buns(self, driver):
        tabs = driver.find_elements(*Locators.tabComponents)

        driver.find_element(*Locators.tabsBlock).click()

        tabs[0].click()
        WebDriverWait(driver, timeout=5).until(EC.visibility_of_element_located(Locators.tabAreaBuns))
    ...
    def test_navigation_tab_sauces(self, driver):
        ...
        tabs[1].click()
        WebDriverWait(driver, timeout=5).until(EC.visibility_of_element_located(Locators.tabAreaSauces))
    ...
    def test_navigation_tab_fillings(self, driver):
        ...
        tabs[2].click()
        WebDriverWait(driver, timeout=5).until(EC.visibility_of_element_located(Locators.tabAreaFillings))
